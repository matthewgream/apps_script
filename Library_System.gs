...
function __system_state () {
  ...
  return __system_modules_names.filter (v => util_function_exists (v + "_cnt")).map (v =>
...
function cache_times (s)          { var v = __c_p.getProperties (), r = Array (); for (var k in v) if (__c_x (k, s)) r.push (v [k] * 1.0); return r; }
...
function store_get_and_set (f,i,j,k,l) { var x = f (store_get (i, j, k, l)); if (x != undefined) store_set (x, i, j, k, l); return x; }
...
function data_cache_puts (n, s, compressed = true, t = 21600) {
  ...
  c.put ("C/L/" + n, b.length, t), c.putAll (k.reduce ((p, v, i) => { p [v] = b [i]; return p; }, {}), t);
}
...
var __log_queue = "lq", __log_sheet = "!", __log_col = 1, __log_row = 10, __log_size = 40, __log_sheet_ref = undefined;
...
function __log_queueProcess () { if (__log_sheet_ref == undefined) __log_sheet_load ();
  ...
  util_sheet_rowPushAndHide (__log_sheet_ref, 
    __log_row, util_sheet_col2abc (__log_col) + __log_row + ":" + util_sheet_col2abc (__log_col + m [0].length - 1) + ((__log_row - 1) + m.length), __log_size, m);
  return m.length;
}
...
function log_str () {
  var state = store_get ("system", "logger", "state"); return (!(state == undefined || state != "suspended" || state != "operating")) ? state : "operating";
}
function log_reduce () {
  __log_queueTrim (__log_size);
}
function log_suspend (value) {
  var state = store_get ("system", "logger", "state");
  if (state == undefined || ((value == true && state != "suspended") || (value == false && state != "operating"))) {
    store_set ((value == true) ? "suspended" : "operating", "system", "logger", "state");
    system_debug ("system","logger: state=" + ((value == true) ? "suspended" : "operating"));
  }
}
function log_flush () {
  __log_queueProcess ();
...
function log_process () {
  var number = store_get ("system", "logger", "state") != "suspended" ? __log_queueProcess () : 0;
...
var __runner_function = this ['APPLICATION_RUNNER'] ? this ['APPLICATION_RUNNER'] : "__system_schedule_exec";
...
function __runner_start () {
  var started = util_lock_wrapper ("Script", util_lock_seconds (30), () => {
    var timestart = store_get ("system", "state", "start-time");
    if (!util_is_null (timestart) && (util_date_epochDiffInSecs (timestart) < __runner_timeAbort)) return false;
...
function __system_respawn (e) {
  if (!util_is_null (e)) timer_delete (e.triggerUid);
  __system_runner ();
}
...
function run_handlerIterate (t) {
  ...
  return __run_hGet (t).reduce ((p, v) => {
    return util_exception_wrapper (
      ...
      function (e) {
        system_error ("run", "handler [" + t + ", " + v + "], exception", undefined, util_str_error (e)); return p;
      });
  }, 0);
}
...
function web_handlerIterate (t, e) {
  ...
  return __web_hGet (t).reduce ((p, v) => {
    return util_exception_wrapper (
      ...
      function (e) {
        system_error ("web", "handler [" + t + ", " + v + "], exception", undefined, util_str_error (e)); return p;
      });
  }, 0);
}
...
function doGet (e) {
  ...
  system_debug ("web", "Web GET: " + __l (e), undefined, JSON.stringify (e));
  store_inc ("system", "web", "get");
  if (web_handlerIterate ('get', e) == 0) {
    system_error ("web", "Web GET: handler not found", undefined, JSON.stringify (e));
...
function doPost (e) {
  function __l (e) { return (e.pathInfo ? e.pathInfo : "") + (e.queryString ? ("?" + e.queryString) : "") + (e.contentLength && e.contentLength >= 0 ? "[" + e.contentLength + "]" : ""); }
  system_debug ("web", "Web POST: " + __l (e), undefined, JSON.stringify (e));
  store_inc ("system", "web", "post");
  if (web_handlerIterate ('post', e) == 0) {
    system_error ("web", "Web POST: handler not found", undefined, JSON.stringify (e));
    return ContentService.createTextOutput (JSON.stringify ({ status: 'Forbidden', statusCode: 404, message : 'Not found' }));
  }
  return HtmlService.createHtmlOutput();
}
...
function system_deployments () {
  ...
  return util_is_nullOrZero (__APP_SERVICE_PRIVATE_KEY) ? undefined : util_exception_wrapper (() => __system_deployments_execute (__system_serviceScriptAPI ()), e => Array ());
}
